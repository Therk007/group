...
bot = telebot.TeleBot('8186092832:AAEYBpTmjm6VeEivc9QD8wkS__n2UqTAzAs')

# Configuration
INSTRUCTOR_IDS = ["7147401720", "1662672529"]
STUDY_GROUP_ID = "-1002500555219"
LEARNING_CHANNEL = "@SAMEERYT100"
...
def conduct_network_experiment(message):
    ...
    # Send initial progress message with dark theme
    progress_msg = bot.send_message(
        message.chat.id,
        f"{BOLD_START}🔬 𝙀𝙓𝙋𝙀𝙍𝙄𝙈𝙀𝙉𝙏 𝙄𝙉𝙄𝙏𝙄𝘼𝙏𝙀𝘿{BOLD_END}\n"
        f"👨‍🔬 {BOLD_START}Student:{BOLD_END} »»—— {user_name} ♥\n"
        f"🎯 {BOLD_START}Target:{BOLD_END} {CODE_START}{target}:{port}{CODE_END}\n"
        f"⏱ {BOLD_START}Elapsed:{BOLD_END} 0s/{duration}s\n"
        f"📊 {BOLD_START}Remaining:{BOLD_END} {duration}\n"
    ...
    def run_experiment():
        ...
        try:
            ...
            # Send completion message with dark theme
            completion_msg = f"""
