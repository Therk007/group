...
# Admin ID
ADMINS = {1662672529}  # Add more admin IDs if needed
OWNER_ID = 1549748318
MAX_CONCURRENT_ATTACKS = 7  # Set the maximum number of concurrent attacks
...
REQUIRED_CHANNELS = ["@RAJOWNER9090", "@HUNTERxPARTNER99"]  # Add required channels here
...
def handle_feedback(message):
    ...
    if user_id in pending_feedback:
        ...
        bot.reply_to(message, "✅ Feedback received! You can start another alone.")
    else:
...
def handle_attack(message):
    ...
    if user_id in banned_users and current_time < banned_users[user_id]:
        ...
        bot.reply_to(message, f"🚫 You are banned for {remaining_ban // 15} minutes!")
        return
    ...
    if not is_user_member(user_id):
        ...
        bot.reply_to(message, 
                   ...
                   f"➡️ [Join Channel 1](https://t.me/RAJOWNER9090)\n"
                   f"➡️ [Join Channel 2](https://t.me/HUNTERxPARTNER99)",  parse_mode="Markdown", disable_web_page_preview=True
                )
        return
    ...
    if user_id in cooldowns and current_time < cooldowns[user_id]:
        ...
        bot.reply_to(message, f"⏳ You must wait {remaining_cd} seconds before starting a new alone!")
        return
    
    if len(active_attacks) >= MAX_CONCURRENT_ALONE:
        bot.reply_to(message, "⏳ Maximum concurrent alones reached! Please wait.")
        return

    command = message.text.split()
    if len(command) != 4:
        bot.reply_to(message, "⚠️ Usage: /alone <ip> <port> <time>", parse_mode="Markdown")
        return
    ...
    try:
        ...
        if port in BLOCKED_PORTS:
            bot.reply_to(message, f"❌ Port `{port}` is blocked and cannot be aloned!", parse_mode="Markdown")
            return
        
        # Limit alone duration to 150 seconds if it exceeds
    ...
    # Log attack details
    logging.info(f"Alone started by {user_id} on {target}:{port} for {time_duration} seconds")
    ...
    def run_attack():
        ...
        try:
            ...
            bot.reply_to(message, 
                      ...
                      f"🔌 PORT: {port}\n"
                      f"⏰ TIME: {time_duration} seconds\n", parse_mode="Markdown")
            logging.info(f"Alone finished by {user_id} on {target}:{port}")
